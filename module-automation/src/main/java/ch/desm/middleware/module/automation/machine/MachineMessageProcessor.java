...
public class MachineMessageProcessor extends ComponentMessageProcessorUbw32Base<MachineService> {

	private static Logger LOGGER = Logger.getLogger(MachineMessageProcessor.class);

	/**
	 * @param messages
	 */
	@Override
	public void processBrokerMessage(MachineService service, List<MessageCommon> messages) {
		for(MessageCommon message : messages){
			processBrokerMessage(service, message);
		}
	}

	public void processEndpointMessage(MachineService service, EndpointUbw32Message ubw32Message){
	...
	private void processBrokerMessage(MachineService service, MessageCommon element){
	...
	private void processBrokerMessageZusiFahrpult(MachineService service, MessageCommon message) {
	...
	private void processBrokerMessagePetrinetRe420(MachineService service, MessageCommon message) {
		String globalId = message.getGlobalId();
		String key = service.getMapPetrinetRe420().getKeyForValue(globalId);
		delegateToEndpoint(service.getEndpoint(), service.getMapDigital(), service.getMapAnalog(), key, message);
	}

	private void processBrokerMessageManagement(MachineService service, MessageCommon message) {
		if (isInitProcessMessage(message)) {
			processInitEndpoint(service.getEndpoint(), message);
		}
	}

	private void processInitEndpoint(MachineEndpointUbw32 endpoint, MessageCommon element){
