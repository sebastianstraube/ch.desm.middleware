...
import sebastianstraube.connectx.core.communication.endpoint.tcp.EndpointTcpClient;
...
public class ZusiEndpointTcpClient extends EndpointTcpClient {
    ...
    public void sendCommand(String globalId){
        try {
            ...
            String stream = service.getCodec().encode(n);
            this.send(stream);
        } catch (IOException e) {
            LOGGER.log(Level.ERROR, e);
        } catch (Exception e) {
    ...
    public void sendCommandSignalAspect(){
        try {
            //String streckenModul, int referenzNr, int signalModus, int signalMatrixZeile, int signalMatrixSpalte
