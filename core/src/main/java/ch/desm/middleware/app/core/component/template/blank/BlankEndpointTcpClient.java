...
public class BlankEndpointTcpClient extends EndpointTcpClient {

    private static Logger LOGGER = Logger.getLogger(BlankEndpointTcpClient.class);

    private BlankService service;
    private BlankEndpointTcpClientThread messageHandler;

	public BlankEndpointTcpClient(BlankService service, String ip, int port, String topic, String name) {
		super(ip, port, name);
        this.registerEndpointListener();
        this.service = service;
        this.messageHandler = new BlankEndpointTcpClientThread(service, topic);
